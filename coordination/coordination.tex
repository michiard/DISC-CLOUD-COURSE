...
\frame {\frametitle{Consistency models}
	...
	\begin{itemize}
		\item Eventual consistency: after a successful write, every read from the distributed system will \emph{eventually} return the written value, if no new updates are made to it
...
\begin{frame}[fragile]
	...
	Each node does a \emph{joinGroup}:
	\begin{verbatim}
		create(GROUP_PATH + proc_name,
		       [address], EPHEMERAL)
	\end{verbatim}

	Clients call \emph{getMembers}:
	\begin{verbatim}
