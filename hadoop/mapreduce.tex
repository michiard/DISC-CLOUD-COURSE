...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   \begin{center}
      \framebox{\includegraphics[scale=0.3]{./Figures/mapreduce_schema}}      
    \end{center}
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \begin{itemize}
  \item \textbf{FIFO Scheduler (default behavior)}
    \begin{itemize}
    \item Each job uses the whole cluster\footnote{To be precise, all required slots are assigned to highest priority job. Excess capacity can be given to other jobs.}
    \item Not suitable for shared, production-level cluster
      \begin{itemize}
      \item Long jobs monopolize the cluster
      \item Short jobs can hold back and have no guarantees on
        execution time
      \end{itemize}
    \end{itemize}
  ...
  \item \textbf{Fair Scheduler}
    \begin{itemize}
    \item Every user gets a fair share of the cluster capacity over
      time
    \item Jobs are placed into pools, one for each user
      \begin{itemize}
      \item Users that submit more jobs have no more resources than
        oterhs
      \item Can guarantee minimum capacity per pool
      \end{itemize}
    \item Supports {\color{red}preemption}
      % \begin{itemize}
      % \item Tasks in pools running over capacity can be killed
      % \item Slots dedicated to tasks running under capacity
      % \end{itemize}
    \end{itemize}

    \vspace{20pt}

  \item \textbf{Capacity Scheduler}
    \begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \begin{beamerboxesrounded}[shadow=true]{}
    In the real world, code is buggy, processes crash and machine
    fails
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   \begin{center}
      \framebox{\includegraphics[scale=0.4]{./Figures/shuffle_map}}      
    \end{center}
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \begin{itemize}
  \item \textbf{The output of a map task is not simply written to
      disk}
    \begin{itemize}
  ...
  \item \textbf{Circular memory buffer}
    ...
    \item Threshold based mechanism to {\color{red}spill} buffer
      content to disk
    \item Map output written to the buffer {\color{red}while} spilling
      to disk
  ...
  \item \textbf{Disk spills}
    ...
    \item Output data is partitioned corresponding to the reducers they
      will be sent to
    \item Within each partition, data is sorted ({\color{red}in-memory})
    \item Optionally, if there is a combiner, it is executed just
      after the sort phase ({\color{red}WHY?})
    \end{itemize}
  \end{itemize}
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \item \textbf{More on spills and memory buffer}
    \begin{itemize}
    \item Each time the buffer is full, a {\color{red}new} spill is
      created
    \item Once the map task finishes, there are many spills
    \item Such spills are merged into a single partitioned and sorted
      output file
    \end{itemize}

    \vspace{40pt}

  \item \textbf{The output file partitions are made available to
      reducers over HTTP}
    \begin{itemize}
    \item There are 40 (default) threads dedicated to serve the file
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   \begin{center}
      \framebox{\includegraphics[scale=0.3]{./Figures/spill_partition}}      
    \end{center}
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   \begin{center}
      \framebox{\includegraphics[scale=0.4]{./Figures/data_to_map}}      
    \end{center}
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame {\frametitle{The relationship between \texttt{InputSplit} and
    HDFS blocks}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   \begin{center}
      \framebox{\includegraphics[scale=0.4]{./Figures/split_block}}      
    \end{center}
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame {\frametitle{\texttt{FileInputFormat} and Friends}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \begin{itemize}
  \item \textbf{\texttt{TextInputFormat}}
    \begin{itemize}
    \item Treats each \texttt{newline}-terminated line of a file as a
      value
    \end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame {\frametitle{Filtering File Inputs}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \begin{itemize}
  \item \textbf{\texttt{FileInputFormat} reads all files out of a specified
    directory and send them to the mapper}

\vspace{40pt}


\item \textbf{Delegates filtering this file list to a method
    subclasses may override}
  \begin{itemize}
  \item Example: create your own ``\texttt{xyzFileInputFormat}'' to read \texttt{*.xyz}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \begin{itemize}
  \item \textbf{Each \texttt{InputFormat} provides its own
      \texttt{RecordReader} implementation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame {\frametitle{Input Split Size}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \begin{itemize}
  ...
  \item \textbf{Record readers receive file, offset, and length of
      chunk}
    \begin{itemize}
    \item Example
    \end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   \begin{center}
      \framebox{\includegraphics[scale=0.4]{./Figures/reduce_to_data}}      
    \end{center}
}
